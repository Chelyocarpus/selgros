...
PDF_PATH = Path(
    os.environ.get(
        "ANALYZE_PDF_PATH",
        r"d:\Downloads\ad0385840A6EA92-A03A-4C04-B6B2-F39EFFDBF697}1.pdf",
    )
)
...
if not PDF_PATH.exists():
    ...
    print(f"Please set the ANALYZE_PDF_PATH environment variable or place the PDF at the default location.", file=sys.stderr)
    sys.exit(1)

if not PDF_PATH.is_file():
    print(f"Error: Path exists but is not a file: {PDF_PATH}", file=sys.stderr)
    sys.exit(1)
...
try:
    with pdfplumber.open(str(PDF_PATH)) as pdf:
        ...
        if tables:
            output_file = Path(
                os.environ.get(
                    "ANALYZE_PDF_OUTPUT",
                    r"d:\selgros\protokoll2qr\pdf_table_data.json",
                )
            )
            try:
    ...
    print(f"Error: PDF file is corrupted or has invalid syntax: {e}", file=sys.stderr)
    sys.exit(1)
except Exception as e:
    print(f"Error: Unexpected error while processing PDF: {e}", file=sys.stderr)
    sys.exit(1)
